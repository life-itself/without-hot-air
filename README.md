...
```
index.md              # the book's front page and table of contents
chap01.md ... chapK.md # chapters, at the repo root
preface.md, acknowledgments.md, bibliography.md, author.md, charts.md
assets/               # all 384 figures from the book
without-hot-air.epub  # original epub of the book
extract.py            # regenerates the markdown from the epub
```

Published at https://withouthotair.org via [Flowershow](https://flowershow.app).
The repo root is the publish root, so the markdown files *are* the site.
...
* Chapters are written to the repo root and figures to `assets/`

Fixes belong in `extract.py`, never in the generated chapter markdown — the
chapters are output, and hand-editing them means the next regeneration silently
loses the fix.

Structural checks (links resolve, figures exist, frontmatter parses):
